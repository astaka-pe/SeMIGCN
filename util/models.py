...
def random_rotate(pos: Union[torch.Tensor, np.ndarray], seed: None) -> torch.Tensor:
    if type(pos) == np.ndarray:
        pos = torch.from_numpy(pos)
    if seed:
        np.random.seed(seed)
    rx, ry, rz = np.random.uniform(size=[3]) * 2 * np.pi
    Rx = np.array([[1, 0, 0],
    ...
    Rz = np.array([[np.cos(rz), -np.sin(rz), 0],
                   ...
                   [0, 0, 1]])
    Rm = np.dot(Rz, np.dot(Ry, Rz))
    Rm = torch.from_numpy(Rm).float()
    new_pos = torch.mm(Rm, pos.T.float()).T
    return new_pos
